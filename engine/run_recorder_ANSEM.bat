@echo off
REM ANSEM - HMM candidate, generated from measurements (2026-09-14).
REM CLI --symbol is the HL leg (ANSEM); the Lighter leg (ANSEM)
...
python main.py --record-only --symbol ANSEM --hedge lighter --config config_ANSEM.yaml --no-dashboard >> logs\ANSEM\runner.log 2>&1
