@echo off
REM OPENAI - HMM candidate, generated from measurements (2026-09-14).
REM CLI --symbol is the HL leg (OAI); the Lighter leg (OPENAI)
...
python main.py --record-only --symbol OAI --hedge lighter --config config_OPENAI.yaml --no-dashboard >> logs\OPENAI\runner.log 2>&1
