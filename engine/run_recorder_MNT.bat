@echo off
REM MNT - HMM candidate, generated from measurements (2026-09-14).
REM CLI --symbol is the HL leg (MNT); the Lighter leg (MNT)
...
python main.py --record-only --symbol MNT --hedge lighter --config config_MNT.yaml --no-dashboard >> logs\MNT\runner.log 2>&1
