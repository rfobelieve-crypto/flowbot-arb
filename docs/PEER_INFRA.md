...
## 七之二、同一個作者的另一支（2026-09-05 補讀）

使用者放進來的 `perp-dex-tools-main`——**同一個作者**（`@yourQuantGuy`，
推薦碼與 entropy-arb 一致），八個交易所各自對 Lighter 對沖，
`--fill-timeout 5` 的 post-only → 成交 → 對沖，**就是 B3 的形狀**。

全本讀完的結論在 `PEER_PERP_DEX_TOOLS.md`（2026-09-05 修訂：第一次只讀了
`hedge_mode_edgex.py` 一支就下家族結論，**兩條是錯的**，已更正）。

**這個家族其實是兩種策略**：七支是「掛 post-only → 成交才對沖」（B3 的形狀），
而 **`hedge_mode_grvt_v2.py` 是兩腿同時吃單的跨所套利——就是我們的
`_execute`**。第一次讀漏掉它，所以漏掉了下面這一條。

**唯一真正值得帶回來的東西**：`grvt_v2:1290-1297` 的**中樞是最近 1000 筆
價差的滾動中位數**，帶是它上下 2 bps，而且樣本湊滿之前不交易。
同一個作者的兩支程式，我們的上游用設定檔常數 `midline_bps`，v2 用滾動中位數
——**他自己從固定走到了自適應**。那正好是 `NBIS_REGIME_20260903.md` 那個
15 bps 階躍的第三個選項。**但那是策略變更，09-06 過閘之後才評估。**

**它有而我們沒有的是廣度**：十一個連接器，全部對 Lighter。那不改變
`VENUES.md` 第三層的決定，只是把「真要接的時候從哪裡開始」變成一個已答的問題。

## 八、仍然開著的（2026-09-04 晚更新）

- ~~**波動熔斷**~~ ✅ **已做**（`entropy_arb/volatility.py`）。這次研究確實
  沒給現成答案，所以設計是自己的，理由寫在那支檔案的檔頭：量波峰-波谷幅度
  不量標準差（會虧錢的是「讀完簿口到第二腿成交之間走了多遠」，而安靜視窗裡
  的單一跳空正是標準差抹平的形狀）；是 PAUSE 不是 HALT（波動會自己好，
  每次新聞都要人工重啟的熔斷一週內會被關掉）；永不暫停對沖與平倉。
- ~~**B3 掛單路徑**~~ ✅ **已做**，規格就是上面第七節，三條「必須」逐條的
  驗證與四處自審修正見 `B3_MAKER_AUDIT.md`。
- **強平偵測**——目前仍只能靠對帳事後發現。**還開著。**
- **新開的一項**：掛單路徑的啟動殘單掃描在兩個場館嚴格度不同——Lighter 查
  不到就拒絕啟動，HL 查不到只記 warning。原因是 HL `openOrders` 帶 `dex`
  參數在 HIP-3 上還沒實測。**第一次實盤前要補成對稱的**
  （`B3_MAKER_AUDIT.md` §6.4）。
