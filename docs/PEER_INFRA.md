...
逐段讀完的結論在 `PEER_PERP_DEX_TOOLS.md`。摘要：它的**執行語意比
Hummingbot 正確得多**（撤單輸掉競速有處理、重複成交訊息有守衛、部位每輪
從交易所重讀），我們比它嚴的有三處（部分成交即時對沖、不確定時讀交易所
而不是把猜測寫進部位、對沖腿保證送達）。風控則跟 Hummingbot 同一族——薄。
