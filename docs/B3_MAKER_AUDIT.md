...
2. HL `openOrders` 帶 `dex` 參數在 HIP-3 dex 上的行為（拿不到就記
   warning 並回 0 張，不會誤以為沒有殘單而繼續——但也不會擋住啟動，
   這是目前 HL 側與 Lighter 側的不對稱，見 §6.4）。
...
### 6.4 啟動殘單掃描在兩個場館的嚴格度不同
Lighter 側：查不到 → 例外 → 拒絕啟動。
HL 側：`openOrders` 查不到 → 記 warning → 回 0 → **繼續啟動**。
不對稱是因為 HL 的 `openOrders` 帶 `dex` 參數在 HIP-3 上還沒實測（§5.2）；
一開始就讓它擋住啟動，會在錄價家族之外造成不必要的停機。
**第一次實盤前要把這條補成對稱的。**
