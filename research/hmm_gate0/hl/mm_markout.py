# -*- coding: utf-8 -*-
# 2026-09-15 從 flow_system/research/hl/mm_markout.py 複製（使用者：全部複製過去），原檔仍在原處
...
3. **只有一天行情。** 筆數不是問題，regime 覆蓋才是。所以本支**不下判決**，
   判決與 §1.23b 的 HL 扳機同一天（09-18）換窗重跑，判準不改。
    ...
    python research/hmm_gate0/hl/mm_markout.py
"""
...
ROOT = Path(__file__).resolve().parents[3]  # arb 根目錄
sys.path.insert(0, str(ROOT))

TAPE = Path("D:/flowbot_data/hl/trades")
MID = Path("D:/flowbot_data/hl/mid")
OUT = ROOT / "research" / "hmm_gate0" / "results" / "hl_mm_markout.json"
HORIZONS = (1, 5, 30)                 # 分鐘
