# -*- coding: utf-8 -*-
# 2026-09-15 從 flow_system/research/mrp/spread_arb_paired.py 複製（使用者：全部複製過去），原檔仍在原處
...
ARB_ROOT = os.path.dirname(os.path.dirname(os.path.dirname(HERE)))  # arb 根目錄
sys.path.insert(0, ARB_ROOT)
...
def frozen(s):
    ...
    **這個定義綁在掃描器的取樣間隔上，而那個間隔在 2026-09-13 從 180 秒
    改成 600 秒**（理由是 Lighter 的 per-IP WAF 擋住了 live 引擎的重連，
    見 scanner.py 的 CYCLE_SEC 註解）。所以：

        凍結比例中位   180s 0.019  ->  540s 0.004
        被 FROZEN_CAP 擋掉  17/242  ->  9/242
        翻成通過的八個  ANTH BOT AAOI XIAOMI WEN RGTI BABA ASML

    **那八個不是變好了，是一樣凍結但取樣太粗看不見。** 這一關的用途正好是
    擋掉「報價不動的市場」（不動的簿口是謊），而放慢取樣會讓謊變得看不見
    —— 對一道安全閘門來說是錯的方向。

    所以凡是用這個數字下判決的：要嘛**只用 2026-09-13 之前的列**，要嘛先把
    它改成**以秒定義**（「連續 N 秒沒動」，對取樣率免疫）。核心原則 7：
...
def main():
    ...
    out = os.path.join(ARB_ROOT, "research", "hmm_gate0", "results",
                       "spread_arb_pairs.json")
