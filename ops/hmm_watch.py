...
"""HMM live 引擎的看護 —— 有問題就送 Discord，沒問題就安靜。
    ...
    python ops/hmm_watch.py                    # 自己推導在跑的 live 標的（排程用這個）
    python ops/hmm_watch.py --pair MON --heartbeat   # 強制送一次現況
    python ops/hmm_watch.py --dry              # 印出來但不送

===========================================================================
為什麼在 arb（2026-09-15 從 flow_system 搬過來）
===========================================================================
2026-09-14 寫的時候住在 flow_system，理由是告警管線 `notify.py` 住在那裡，
而 arb 不准 import flow_system。2026-09-15 使用者決定 **HMM 的東西全部搬到
arb、從這邊開 session**，所以投遞層換成 arb 自己的一份（`ops/alert.py`，
憑證也是 arb 自己的 `ARB_DISCORD_WEBHOOK_URL`）。**這支現在一行都不讀
flow_system**；flow_system 的新鮮度看板仍然從外面讀 `status.json` 與
`results/hmm_alert_last.json`，方向是 flow_system -> arb，沒反。
...
ARB = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
...
STATE = os.environ.get("HMM_WATCH_STATE") or os.path.join(
    ARB, "results", "hmm_watch_last.json")
...
def look(pair: str) -> tuple[list, dict]:
    ...
    # HALT 自動恢復的事件（2026-09-15）。動手的是 `engine/tools/halt_recover.py`,
    # 它只寫一份 json;這裡讀去報（兩者 2026-09-15 起同在 arb）。
    # 自動重啟一個會送真單的引擎**必須被看見** —— 一個沒有人知道的自動
    # 恢復,跟沒有恢復一樣危險,只是方向相反。
    hrp = os.path.join(d, "halt_recover.json")
    if os.path.exists(hrp):
        try:
            hrs = json.load(io.open(hrp, encoding="utf-8"))
            last = hrs.get("last") or {}
            if last.get("ts") and now - float(last["ts"]) < 3600:
                probs.append("**已自動重啟**（%.0f 分鐘前）：%s"
                             % ((now - float(last["ts"])) / 60,
                                str(last.get("reason", ""))[:90]))
            if hrs.get("blocked_at") and now - float(hrs["blocked_at"]) < 3600:
                probs.append("**自動恢復預算用完,引擎還 HALT 著** —— "
                             "這不是暫時性問題,要人看")
        except Exception:
            probs.append("halt_recover.json 讀不了 —— 不知道它做過什麼")
...
def _watch_one(pair: str, a, last: dict) -> int:
    ...
    return _emit(pair, msg, "|".join(sorted(probs)), a, last)


def _emit(slot: str, msg: str, key: str, a, last: dict) -> int:
    """狀態轉換才送。`key` 是狀態指紋，`slot` 是它在狀態檔裡的位置。"""
    pair = slot
    ...
    if changed or a.heartbeat:
        r = alert.send(msg, source="hmm_watch")
        print("[送出] delivered=%s tried=%s" % (r.get("delivered"), r.get("tried")))
        if not r.get("delivered"):
            # 送不出去本身是一盞紅燈 —— alert 會寫 results/hmm_alert_last.json，
            # 而 flow_system 的新鮮度看板從外面讀那個旗標。
            print("**告警送不出去** —— 看 results/hmm_alert_last.json",
                  file=sys.stderr)
    else:
        print("[安靜] 狀態沒變，不重複送")
    last[pair] = key
...
BUDGET = os.path.join(ARB, "results", "account_budget.json")
BUDGET_MAX_AGE_SEC = 1800.0   # arb_watchdog 每 5 分鐘寫一次；半小時沒動 = 看門狗沒在跑


def _watch_account(pairs: list, a, last: dict) -> int:
    """帳戶層與「到底有沒有東西在盯」—— 不屬於任何單一標的的那兩件事。

    **為什麼搬到這裡（2026-09-15）**：使用者訂了分工「flowsystem 報 v7，arb 報 HMM」。
    在那之前這兩件是 flow_system 新鮮度看板的兩列（`arb 帳戶額度加總 (B6)` 與
    `HMM 引擎 <pair>` 在推導不出標的時變紅）。看板拿掉那兩列，所以它們要在
    這裡有人報，否則就是一格靜默消失的守衛（mistake.md 2026-09-04）。

      B6    `engine/tools/account_budget.py` 由 arb_watchdog 每 5 分鐘寫；
            ok=False（N 個行程共用一個帳戶、額度加總超過天花板）或檔案過期都報。
      空集  `live_hmm_pairs()` 回空 = 全部被 STOP，**或**註冊表解析壞了。
            兩者這裡分不出來，所以只在狀態轉換時說一次，不是每五分鐘。
    """
    probs = []
    if not pairs:
        probs.append("**沒有在盯任何 live HMM 標的** —— 全部被 STOP，"
                     "或看門狗註冊表解析不出來（兩者這裡分不出來，去看 "
                     "engine/logs/stop/ 與 ops/arb_watchdog.ps1）")
    if not os.path.exists(BUDGET):
        probs.append("**account_budget.json 不存在** —— 帳戶層額度沒人在算")
    else:
        age = time.time() - os.path.getmtime(BUDGET)
        if age > BUDGET_MAX_AGE_SEC:
            probs.append("**帳戶額度 %.0f 分鐘沒更新** —— arb_watchdog 可能沒在跑"
                         % (age / 60))
        try:
            b = json.load(io.open(BUDGET, encoding="utf-8"))
            if not b.get("ok", False):
                probs.append("**帳戶層額度紅**：%s" % str(b.get("reason", ""))[:140])
        except Exception as e:                       # noqa: BLE001
            probs.append("account_budget.json 讀不了：%s" % type(e).__name__)
    head = "🔴 HMM 帳戶層" if probs else "🟢 HMM 帳戶層"
    msg = "\n".join([head] + ["• " + p for p in probs] +
                    ["— 在盯的 live 標的：%s" % (" ".join(pairs) or "無")])
    return _emit("_account", msg, "|".join(sorted(probs)), a, last)
...
def main(argv=None) -> int:
    ...
    # **空集不再提早 return（2026-09-15）。** 以前「推導不出標的」交給
    # flow_system 看板的 HMM 那一列去紅；那一列照「flowsystem 報 v7、arb 報 HMM」
    # 拿掉了，所以這裡自己報。而且每一輪都要寫狀態檔 —— flow_system 看板
    # 靠它的 mtime 判斷「這支看護還活著」，提早 return 會讓沒有 live 標的的
    # 那段時間看起來像看護死了。
    if not a.pair:
