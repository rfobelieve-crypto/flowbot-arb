...
`account_budget.py`、`hmm_watch.py`，以及從外面讀的 flow_system `freshness_board.py`）。

2026-09-15 起住在 `arb/ops/`。flow_system 的看板透過 `research/arb_home.py`
找到這裡再 import —— 讀的方向是 flow_system -> arb，沒有反。
...
# 2026-09-15 從 flow_system/research/ops 搬進 arb/ops：arb 根目錄就是上一層。
ARB = os.path.join(HERE, "..")
